...
namespace bw
{
	void TimeSyncState::Enter(Ndk::StateMachine& fsm)
	{
		...
		m_statusText = stateData.world->CreateEntity();
		...
		ConnectSignal(m_clientSession->OnTimeSyncResponse, this, &TimeSyncState::OnTimeSyncResponse);
	}
	...
	bool TimeSyncState::Update(Ndk::StateMachine& fsm, float elapsedTime)
	{
		...
		if (m_accumulator >= m_nextStepTime)
		{
			if (!m_finished)
			{
				...
				m_clientSession->SendPacket(timeSyncRequest);
	...
	void TimeSyncState::OnTimeSyncResponse(ClientSession* /*session*/, const Packets::TimeSyncResponse& response)
	{
		...
		if (m_results.size() >= DesiredRequestCount)
		{
			...
			m_clientSession->UpdateServerTimeDelta((m_isClientYounger) ? meanDiff : std::numeric_limits<Nz::UInt64>::max() - meanDiff);
