...
namespace bw
{
	...
	m_world(AddWorld()),
	m_mainWindow(AddWindow<Nz::RenderWindow>(Nz::VideoMode(800, 600), "Mon super titre")),
	m_appTime(0),
	...
	m_isMoving(false)
	{
		...
		physics.SetGravity(Nz::Vector2f(0.f, 9.81f * 64.f));
		...
		while (Application::Run())
		{
			...
			if (m_appTime - lastUpdate > 1'000 / 30)
			{
				...
				if (Nz::Keyboard::IsKeyPressed(Nz::Keyboard::Up) && isOnGround)
				{
					...
					burgerPhys.AddImpulse(Nz::Vector2f(dirForce, -400.f * burgerPhys.GetMass()));

					isOnGround = false;
				}

				if (Nz::Keyboard::IsKeyPressed(Nz::Keyboard::Left) && isOnGround)
				{
					auto& burgerPhys = m_burger->GetComponent<Ndk::PhysicsComponent2D>();
					burgerPhys.AddImpulse(Nz::Vector2f(-500.f, -100.f) * burgerPhys.GetMass());

					isFacingRight = false;
					isOnGround = false;
				}

				if (Nz::Keyboard::IsKeyPressed(Nz::Keyboard::Right) && isOnGround)
				{
					auto& burgerPhys = m_burger->GetComponent<Ndk::PhysicsComponent2D>();
					burgerPhys.AddImpulse(Nz::Vector2f(500.f, -100.f) * burgerPhys.GetMass());
